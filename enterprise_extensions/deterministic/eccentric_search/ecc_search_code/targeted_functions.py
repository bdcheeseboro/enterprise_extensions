...
###bounded normal adapted from PTB
...
def BoundedNormal(mu=0, sigma=1, pmin=-1, pmax=1, size=None):
    ...
    class BoundedNormal(parameter.Parameter):
        _prior = parameter.Function(BoundNormPrior, mu=mu, sigma=sigma, pmin=pmin, pmax=pmax)
        ...
        def __repr__(self):
            return '"{}": BoundedNormal({},{}, [{},{}])' \
                    .format(self.name, mu, sigma, pmin, pmax) \
                    + ('' if self._size is None
                        else '[{}]'.format(self._size))
...
class JumpProposal(object):
    ...
    def __init__(self, pta, snames=None, fgw=3e-8, psr_dist=None, 
                 rnposteriors=None, juporbposteriors=None, tprocposteriors=None):
        ...
        if tprocposteriors is not None and os.path.isfile(tprocposteriors):
            with open(tprocposteriors, 'rb') as f:
                self.tprocDistr = pickle.load(f)
                self.tprocDistr_psrnames = [ t.param_name[0].split('_')[0] for t in self.tprocDistr ]
        else:
            self.tprocDistr = None
            self.tprocDistr_psrnames = None
            
        if rnposteriors is not None and os.path.isfile(rnposteriors):
            with open(rnposteriors, 'rb') as f:
                self.rnDistr = pickle.load(f)
                self.rnDistr_psrnames = [ r.param_names[0].astype(str).split('_')[0] for r in self.rnDistr ]
        else:
            self.rnDistr = None
        ...
        if juporbposteriors is not None and os.path.isfile(juporbposteriors):
            with open(juporbposteriors, 'rb', ) as f:
    ...
    def draw_from_ephem_prior(self, x, iter, beta):
        ...
        be_pars = [ p for p in self.pnames if p in  ['d_jupiter_mass', 'd_neptune_mass', 'd_saturn_mass', 'd_uranus_mass', 'frame_drift_rate',
                    'jup_orb_elements_0', 'jup_orb_elements_1', 'jup_orb_elements_2', 'jup_orb_elements_3',
                    'jup_orb_elements_4', 'jup_orb_elements_5']]
        
        myparam = np.random.choice(be_pars)
        idx = 0
    ...
    def draw_from_tproc_prior(self, x, iter, beta):
        
        q = x.copy()
        lqxy = 0
        
        tproc_psrs = [ p.split('_')[0] for p in self.pnames if 'alphas_0' in p]
        
        if len(tproc_psrs) > 0:
            # randomly pick a pulsar
            mypsr = np.random.choice(tproc_psrs)
            my_alpha = np.random.choice(np.arange(0,30))
            myparam = np.random.choice([mypsr + '_red_noise_alphas_'+str(my_alpha)])
            idx = 0
            for i,p in enumerate(self.params):
    ...
    def draw_from_tproc_distr(self, x, iter, beta):

        q = x.copy()
        lqxy = 0

        if self.tprocDistr is not None:

            # randomly choose one of the empirical distributions
            distr_idx = np.random.randint(0, len(self.tprocDistr))


            idx = self.pnames.index(self.tprocDistr[distr_idx].param_name[0])
            q[idx] = self.tprocDistr[distr_idx].draw()

            lqxy = (self.tprocDistr[distr_idx].logprob(x[idx]) -
                    self.tprocDistr[distr_idx].logprob(q[idx]))
        return q, float(lqxy)
    ...
    def draw_from_1713_1909_rnposteriors(self, x, iter, beta):
        
        q = x.copy()
        lqxy = 0
        
        # randomly pick a pulsar
        mypsr = np.random.choice(['J1713+0747', 'J1909-3744'])
    ...
    def draw_from_1713_1909_rnpriors(self, x, iter, beta):
